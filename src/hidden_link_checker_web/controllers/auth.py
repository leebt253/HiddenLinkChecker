...
from hidden_link_checker_web.views.pages import render_login
...
async def welcome(request: Request) -> Response:
    """Keep the old URL compatible while sending authenticated users to the dashboard."""
    try:
        await _api_client(request).get_current_user(request.cookies)
    except httpx.HTTPStatusError as error:
